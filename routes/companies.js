...
const { ensureLoggedIn, userIsAdmin } = require("../middleware/auth");
  ...
  try {
    ...
    if(Object.keys(req.query).length !== 0){
      ...
      if(req.query.minEmployees  > req.query.maxEmployees) {
        throw new BadRequestError("Min Employees must be less than max employees filter.", 400)
      }
...
router.patch("/:handle", userIsAdmin, async function (req, res, next) {
  try {
...
router.delete("/:handle", userIsAdmin, async function (req, res, next) {
  try {
